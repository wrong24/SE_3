...
with st.form("new_resource"):
    ...
    if st.form_submit_button("Add Resource"):
        ...
        response = requests.post("http://localhost:8004/resource", json=resource)
        if response.status_code == 200:
            st.success("Resource added!")
            st.rerun()

# Resource assignment form
response = requests.get("http://localhost:8004/resources")
if response.status_code == 200:
    ...
    if resources:
        ...
        with st.form("assign_resource"):
            ...
            if st.form_submit_button("Assign"):
                ...
                response = requests.post("http://localhost:8004/assign", json=assignment)
                if response.status_code == 200:
...
response_assignments = requests.get("http://localhost:8004/assignments")
...
if len(resources) >= 3 and len(assignments) >= 5:
    if st.button("Complete Exercise"):
        response = requests.post("http://localhost:8004/complete_exercise")
        if response.status_code == 200:

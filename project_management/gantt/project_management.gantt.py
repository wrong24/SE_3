...
with st.form("new_task"):
    ...
    if st.form_submit_button("Add Task"):
        ...
        response = requests.post("http://localhost:8003/task", json=task)
            ...
            with col2:
                if st.button("Update", key=f"update_{task['id']}"):
                    response = requests.put(
                        f"http://localhost:8003/task/{task['id']}",
...
if len(tasks) >= 3 and all(task["progress"] == 100 for task in tasks):
    if st.button("Complete Exercise"):
        response = requests.post("http://localhost:8003/complete_exercise")
        if response.status_code == 200:

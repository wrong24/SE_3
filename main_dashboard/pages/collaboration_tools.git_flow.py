...
def create_commit(branch: str, message: str):
    response = requests.post(
        "http://collaboration_tools:8006/commit",
        json={
            "message": message,
            "branch": branch,
...
def create_branch(name: str, base: str):
    response = requests.post(
        "http://collaboration_tools:8006/create-branch",
...
def merge_branches(source: str, target: str):
    response = requests.post(
        "http://collaboration_tools:8006/merge",
...
if st.button("Refresh Repository State"):
    response = requests.get("http://collaboration_tools:8006/state")
...
if st.session_state.step >= len(steps):
    if st.button("Complete Exercise"):
        response = requests.post("http://collaboration_tools:8006/complete_exercise")
        if response.status_code == 200:

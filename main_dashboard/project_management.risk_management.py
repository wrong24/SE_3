...
with st.form("new_risk"):
    ...
    if st.form_submit_button("Add Risk"):
        ...
        response = requests.post("http://project_management:8005/risk", json=risk)
        if response.status_code == 200:
            st.success("Risk added!")
            st.rerun()

# Display risk matrix
response = requests.get("http://project_management:8005/risks")
if response.status_code == 200:
                ...
                if new_status != risk["status"]:
                    if st.button("Update Status", key=f"update_{risk['id']}"):
                        response = requests.put(
                            f"http://project_management:8005/risk/{risk['id']}",
...
if len(risks) >= 3 and any(r["status"] == "Mitigated" for r in risks):
    if st.button("Complete Exercise"):
        response = requests.post("http://project_management:8005/complete_exercise")
        if response.status_code == 200:

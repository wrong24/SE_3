...
with st.form("new_story"):
    ...
    if st.form_submit_button("Create Story"):
        ...
        response = requests.post("http://localhost:8013/story", json=story)
        if response.status_code == 200:
            st.success("User Story created!")

# Display stories
st.header("User Stories")
response = requests.get("http://localhost:8013/stories")
if response.status_code == 200:
...
if st.button("Complete Exercise"):
    response = requests.post("http://localhost:8013/complete_exercise")
    if response.status_code == 200:

...
if uploaded_file:
    if st.button("Upload"):
        ...
        response = requests.post("http://localhost:8010/upload", files=files)
...
if st.button("Refresh File List"):
    response = requests.get("http://localhost:8010/files")
    if response.status_code == 200:
            ...
            for file in files:
                ...
                with col2:
                    if st.button("Download", key=file):
                        response = requests.get(f"http://localhost:8010/download/{file}")
                        if response.status_code == 200:
                            st.download_button(
                                label="Save File",
...
if st.button("Complete Exercise"):
    response = requests.post("http://localhost:8010/complete_exercise")
    if response.status_code == 200:

...
response = requests.get("http://localhost:8009/list")
if response.status_code == 200:
    documents = response.json()["documents"]
    for doc in documents:
        if st.sidebar.button(doc):
            response = requests.get(f"http://localhost:8009/document/{doc}")
            if response.status_code == 200:
...
if st.button("Complete Exercise"):
    response = requests.post("http://localhost:8009/complete_exercise")
    if response.status_code == 200:

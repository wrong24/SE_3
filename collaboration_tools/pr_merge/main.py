...
response = requests.get("http://localhost:8007/list-prs")
if response.status_code == 200:
    ...
    for pr in prs:
        with st.expander(f"{pr['title']} (#{pr['id']}) - {pr['status']}"):
            ...
            with col1:
                ...
                response = requests.get("http://localhost:8007/code/main")
                if response.status_code == 200:
            ...
            if pr["status"] == "open":
                ...
                with col1:
                    if st.button("Approve", key=f"approve_{pr['id']}"):
                        response = requests.post(
                            f"http://localhost:8007/review-pr/{pr['id']}",
                            params={"approve": True}
                        )
                        if response.status_code == 200:
...
if st.button("Complete Exercise"):
    response = requests.post("http://localhost:8007/complete_exercise")
    if response.status_code == 200:
